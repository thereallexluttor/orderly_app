...
import 'package:orderly_app/payment/paymentOrderPos.dart';
...
if (currentUser == null) {
    ...
    // Por ejemplo, retornar o lanzar una excepción
}
...
// Calcular la suma total solo para el usuario autenticado
double totalAmount = 0.0;
if (groupedItems.containsKey(currentUserUid)) {
    totalAmount = groupedItems[currentUserUid]!.fold(0.0, (sum, item) {
...
print(totalAmount);
  ...
  child: ListView.builder(
    itemCount: 1,//groupedItems.length,
    itemBuilder: (context, index) {
      final photouserKey = groupedItems.keys.elementAt(index);
      final itemsForUser = groupedItems[currentUserUid];
                    ...
                    onPressed: EnableButton ? () {
                      ...
                      print(itemsForUser);
                       ...
                       Navigator.push(
                         context,
                         MaterialPageRoute(
                           builder: (context) => paymentOrderPos( totalAmount, itemsForUser as  List<Map<String, dynamic>>, firestorepath1, firestorepath2
                      //       itemsForUser,
                      //       widget.photoUrl,
                      //       firestorepath1,
                      //       firestorepath2,
 ...
 Stream<int> countTotalOrderedProductsStream() {
    ...
    User? currentUser = FirebaseAuth.instance.currentUser;
    if (currentUser == null) {
        return Stream.value(0); // Si el usuario no está autenticado, retornar 0
    }
    String currentUserUid = currentUser.uid;
    ...
    return FirebaseFirestore.instance.collection(firestorePath1).doc(firestorePath2).snapshots().map((snapshot) {
        if (snapshot.exists) {
            ...
            // Contar cada producto si el UserId coincide con el UID del usuario autenticado
            orderData.forEach((key, items) {
                if (items is List) {
                    for (var item in items) {
                        if (item is Map<String, dynamic> && item.containsKey('productName') && item['UserId'] == currentUserUid) {
                            totalCount++;
                        }
                    }
                }
            ...
            return totalCount; // Retornar el conteo total de productos
        } else {
